...
usockets-macos:
	cd $(USOCKETS_PATH) && $(CC) -mmacosx-version-min=10.14 -I/usr/local/opt/openssl/include -I ./src -DLIBUS_USE_OPENSSL -lssl -lcrypto -pthread -fPIC -std=c11 -O3 -c src/*.c src/eventing/*.c src/crypto/*.c
	cd $(USOCKETS_PATH) && $(CXX) -stdlib=libc++ -mmacosx-version-min=10.14 -I/usr/local/opt/openssl/include -I -DLIBUS_USE_OPENSSL -lssl -lcrypto -lstdc++ -pthread -fPIC -std=c++17 -O3 -c src/crypto/*.cpp
...
macos-exec-arm64:
	$(CXX) -stdlib=libc++ -target arm64-apple-macos11 -I/usr/local/opt/openssl/include -I ./src -I $(UWS_PATH)/src -I $(USOCKETS_PATH)/src -I -DLIBUS_USE_OPENSSL -lssl -lcrypto -lstdc++ -pthread -fPIC -std=c++17 -c -O3 ./src/*.cpp 
	$(CXX) -stdlib=libc++ -target arm64-apple-macos11 *.o $(USOCKETS_PATH)/usockets_macos_arm64.a -lssl -lcrypto -lstdc++ -pthread -fPIC -lz -std=c++17 -o $(EXEC_NAME)

usockets-macos-arm64:
	cd $(USOCKETS_PATH) && $(CC) -target arm64-apple-macos11 -I/usr/local/opt/openssl/include -I ./src -DLIBUS_USE_OPENSSL -lssl -lcrypto -pthread -fPIC -std=c11 -O3 -c src/*.c src/eventing/*.c src/crypto/*.c
	cd $(USOCKETS_PATH) && $(CXX) -stdlib=libc++ -target arm64-apple-macos11 -I/usr/local/opt/openssl/include -I -DLIBUS_USE_OPENSSL -lssl -lcrypto -lstdc++ -pthread -fPIC -std=c++17 -O3 -c src/crypto/*.cpp
